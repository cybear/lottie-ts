...
interface PolynomialBezierInstance {
  a: Point2D;
  b: Point2D;
  c: Point2D;
  d: Point2D;
...
function floatZero(f: number): boolean {
  return Math.abs(f) <= 0.00001;
}

function lerp(p0: number, p1: number, amount: number): number {
  return p0 * (1 - amount) + p1 * amount;
}

function lerpPoint(p0: Point2D, p1: Point2D, amount: number): Point2D {
...
function lineIntersection(start1: Point2D, end1: Point2D, start2: Point2D, end2: Point2D): Point2D | null {
  const v1: Point3D = [start1[0], start1[1], 1];
  const v2: Point3D = [end1[0], end1[1], 1];
  const v3: Point3D = [start2[0], start2[1], 1];
  const v4: Point3D = [end2[0], end2[1], 1];

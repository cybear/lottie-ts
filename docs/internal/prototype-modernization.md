...
- **Renderer-family bases:** **`SVGBaseElement`** — `@ts-nocheck` removed; mixin fields declared; `defs` as `SVGDefsElement`; filter handle as `SVGElement` where `createNS` returns a broad type. **`HBaseElement`** — `@ts-nocheck` removed; HTML layer + mask/effect fields; `destroyBaseElement` assigned on `prototype` via a typed assertion so it aliases `destroy`. **`CVBaseElement`** — `@ts-nocheck` removed; `CanvasTransformCanvas` import; `operationsMap` / blend mode / `createCanvas` casts; `loadLumaCanvas?.` / `getLumaCanvas!`; `DocumentWithProxy` for `_isProxy`; `mHelper` on `prototype` via typed assignment; `searchEffectTransforms` **only** `declare`d (no class body override vs `TransformElement`).

...
- **Expression stack (incremental):** **`Expressions.ts`**, **`expressionHelpers.ts`**, **`InterfacesProvider.ts`** typed; callers cast **`ExpressionManager`** default export to **`ExpressionManagerApi`** (**`resetFrame`**, **`initiateExpression`**) because **`ExpressionManager.ts`** is still **`@ts-nocheck`**. **`LayerInterface`**, **`MaskInterface`**, **`CompInterface`**, **`EffectInterface`**, **`ShapeInterface`**, **`TextInterface`**, **`TransformInterface`**, **`PropertyInterface`**, **`ProjectInterface`**, **`FootageInterface`**, **`ShapePathInterface`**, **`ExpressionValue`**, **`ExpressionValueFactory`**, **`ExpressionTextPropertyDecorator`**, **`TextSelectorPropertyDecorator`**, **`PropertyGroupFactory`** remain **`@ts-nocheck`** (~350+ strict errors if stripped without per-parameter **`any`**).
- **Text utilities:** **`TextAnimatorDataProperty`** — **`AnimatorPropsJson`**, **`PropOrStub`**; **`TextSelectorProperty`** — **`TextSelectorPropFactory`** with **`AnimProp`** / **`Record<string, unknown>`** payload, **`getMult` / `getValue`** signatures. **`TextProperty`** and **`TextAnimatorProperty`** still **`@ts-nocheck`** (large surface; depend on typed **`PropertyFactory`**).

...
**Vitest / Rolldown:** **`package.json` `engines`** includes **`^21.7.0`** so anyone on Node **21** gets a clear floor (**`util.styleText`** landed in **21.7**; **20.19** already bundles the LTS backport). Rolldown (via Vitest) **statically imports** **`styleText`** from **`node:util`**, so **21.5** and similar throw at **`npm test`**. Vitest’s own **`package.json` `engines`** omit the Node **21** line entirely—CI uses **20.19 / 22.12 / 24 / 25**; for unit tests prefer those or **≥21.7** if you stay on 21.

...
- **Broad `@ts-nocheck` removal:** Still opted out: **`worker_wrapper.ts`**, **`animation/AnimationItem.ts`**, **`animation/AnimationManager.ts`**, and **`src/modules/*`** (`full`, `full_worker`, `svg`, `canvas`, `html`, `*_light`, `main`). **`main.ts`**, **`module.ts`**, **`renderers/CanvasRendererWorkerOverride.ts`**, **`animation/AnimationItemWorkerOverride.ts`**, and **`animation/AnimationManagerWorkerOverride.ts`** are TypeScript-checked (see Track A bullets). Peel off **small vertical slices** with `npx tsc --noEmit` after each file or small group. Under **`src/utils/expressions/`** there is **no** `@ts-nocheck` left (file-level **`eslint-disable @typescript-eslint/no-explicit-any`** instead).

...
Call sites live mostly on **destination** layer/comp constructors. The helper itself is [`functionExtensions.ts`](../../src/utils/functionExtensions.ts). As of this revision, `extendPrototype(` appears in roughly **a dozen** element modules. **`RenderableDOMElement`**, **`ExpressionPropertyDecorator`**, and **`ICompElement`** no longer call `extendPrototype`. **`CVCompBaseElement`** only subclasses `BaseRenderer` and does **not** call `extendPrototype`.

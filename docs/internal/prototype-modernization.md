...
This document supports Track A (strict typing) and Track B (ES classes) modernization. *Last updated: 2026-03-23 — **`worker_wrapper.ts`**, **`animation/AnimationItem.ts`**, **`animation/AnimationManager.ts`**, and all **`src/modules/*.ts`** entry bundles are TypeScript-checked (no `@ts-nocheck`); **`worker_wrapper.ts`** is listed in **`eslint.config.mjs`** with **`@typescript-eslint/no-explicit-any` off** (proxy DOM/canvas + message bridge). Bundle files that call **`registerEffect`** use a typed **`as (id, effect: any, …) => void`** alias so strict registration matches real effect ctor variance. **`full_worker.ts`** is an empty module stub (`export {}`). **All `extendPrototype(...)` call sites in `src/` destination modules are migrated off the helper; only the helper implementation remains in `src/utils/functionExtensions.ts` (plus the worker-local copy in `worker_wrapper.ts`).** `CVShapeElement` holds the canvas shape pipeline (`createStyleElement` through `renderStroke`, plus `renderInnerContent`) on the `class`; **`createContent`** and **`destroy`** use save-restore vs **`CVBaseElement`**; only **`initElement`** (from **`RenderableDOMElement`**), **`transformHelper`**, and **`dashResetter`** stay **`prototype`** assignments after composition wiring. `CVSolidElement` keeps `renderInnerContent` on the `class`. Compositions: `SVGCompElement`, `CVCompElement`, and `HCompElement` define `createComp` on the `class` (`HCompElement` also `addTo3dContainer`); `CVCompElement` keeps `renderInnerContent` and `destroy` on the `class` (save-restore); `HCompElement` keeps `createContainerElements` on the `class` (save-restore + `_createBaseContainerElements`). `HShapeElement` keeps bbox helpers, **`createContent`**, and **`renderInnerContent`** on the **`class`**; after composition wiring, **`_renderShapeFrame`** is set to the mixed-in SVG **`renderInnerContent`**, then HTML **`createContent`** / **`renderInnerContent`** are restored; shared **`shapeBoundingBox`** / **`tempBoundingBox`** stay on **`prototype`**. `SVGShapeElement` keeps the pipeline on the `class`; `renderInnerContent` / `destroy` use save-restore vs `RenderableDOMElement`; `initSecondaryElement` / `buildExpressionInterface` are class methods; `identityMatrix` remains one shared `prototype` matrix. `SVGTextLottieElement` / `HTextElement` use save-restore for text `renderInnerContent` (and SVG text `sourceRectAtTime`). `CVImageElement` uses save-restore for `createContent` / `destroy` vs `CVBaseElement`; `HImageElement` does the same for `createContent` vs `HSolidElement`. **`ISolidElement` now extends `IImageElement` directly** (no local composition helper call, no save-restore for `createContent`). **`ICompElement` now copies trait descriptors explicitly, while retaining save-restore for `initElement` / `prepareFrame` / `renderInnerContent` / `destroy`.** **`FootageElement`, layer `AudioElement`, `NullElement`, and `HCameraElement` now extend `BaseElement` directly and wire trait methods via explicit descriptor assignment.** Track A: **`CVImageElement`**, **`CVSolidElement`**, **`CVTextElement`**, and **`CVShapeElement`** no longer use **`@ts-nocheck`**; **`CanvasRendererBase`** layer factories narrow **`RendererLayerData`** / **`globalData`** with **`as unknown as`** into **`lottieRuntime`** canvas types (**`GlobalDataCanvasImage`**, **`GlobalDataCanvasLayer`**, **`GlobalDataCanvasText`**, **`CanvasRenderer2D`**). Where the merged stack overrides base behavior, some elements still rely on explicit **`prototype` assertions** (**`SVGShapeElement`**, **`IImageElement`**). **`RenderableDOMElement`** now extends **`RenderableElement`** directly (no IIFE / `createProxyFunction` stack), and **`RenderableElement.hide()` / `show()`** are concrete stubs so subclass overrides type-check. Earlier slices: etc.*
...
[`src/utils/functionExtensions.ts`](../../src/utils/functionExtensions.ts) copies **own property descriptors** from each source constructor’s prototype chain (excluding `Object.prototype`) onto the destination’s `prototype`, in **base → derived** order per source. Later source entries still overwrite earlier keys on collision—**order matters**.
...
## Historical `extendPrototype` call sites by category
...
| `RenderableDOMElement` | `RenderableElement` via `class extends`                                  | `elements/helpers/RenderableDOMElement.ts` |
...
Call sites were historically on **destination** layer/comp constructors. As of this revision, `src/` has **no remaining destination `extendPrototype(...)` call sites** (confirm with `rg "extendPrototype\\(" src --glob '*.ts' -l`); only the helper definition in [`functionExtensions.ts`](../../src/utils/functionExtensions.ts) and the worker-local helper copy remain.
...
**`NullElement`:** now **`class extends BaseElement`** with explicit descriptor wiring for **`FrameElement`**, **`TransformElement`**, and **`HierarchyElement`** methods. **`sourceRectAtTime`** stays restored after wiring so Null’s local method wins over `BaseElement`’s noop.

**`ISolidElement`:** now a direct **`class extends IImageElement`** migration; solid keeps its own **`createContent`** override without save-restore because normal inheritance order already prefers the subclass method.

**`ICompElement` / image:** **`ICompElement`** now uses explicit descriptor-copy composition (no local **`extendPrototype`** call) and keeps save-restore for **`initElement`**, **`prepareFrame`**, **`renderInnerContent`**, and **`destroy`** so class behavior still overrides merged trait defaults. **`IImageElement`** restores **`sourceRectAtTime`** where the merged stack would overwrite.

**`FootageElement` / layer `AudioElement`:** both now use **`class extends BaseElement`** and explicit post-class descriptor wiring for **`RenderableElement`** + **`FrameElement`** methods (`initRenderable`, `prepareRenderableFrame`, `initFrame`, `prepareProperties`, etc.), replacing the old **`extendPrototype([RenderableElement, BaseElement, FrameElement], …)`** calls. **`FootageElement`** still restores **`initExpressions`**; layer **`AudioElement`** still restores **`initExpressions`** and **`sourceRectAtTime`**.

**`HCameraElement`:** now **`class extends BaseElement`** with explicit descriptor wiring for **`FrameElement`** + **`HierarchyElement`** methods (`initFrame`, `prepareProperties`, `initHierarchy`, `checkParenting`, etc.), replacing the old **`extendPrototype([BaseElement, FrameElement, HierarchyElement], …)`** call.
...
- **`NullElement`:** `@ts-nocheck` removed; constructor + instance methods typed; now `extends BaseElement` + explicit `FrameElement`/`TransformElement`/`HierarchyElement` descriptor wiring (no local `extendPrototype` call).
...
- **`FootageElement`:** `@ts-nocheck` removed; `RefIdLayerData`, `ImageLoaderLike`; now `extends BaseElement` + explicit `RenderableElement`/`FrameElement` descriptor wiring (no local `extendPrototype` call).
- **`AudioElement`:** `@ts-nocheck` removed; `AudioLayerData`, `AudioControllerLike` / time+level prop typing; now `extends BaseElement` + explicit `RenderableElement`/`FrameElement` descriptor wiring (no local `extendPrototype` call).
...
- **`ICompElement` (`CompElement.ts`):** `@ts-nocheck` removed; `CompLayerData`, `CompChildElement`, time-remap typing, mixin `declare`s for comp state; now uses explicit descriptor-copy composition (no local `extendPrototype` call).
...
- **Mask stack:** **`mask.ts` (`MaskElement`)** and **`CVMaskElement`** — `@ts-nocheck` removed; **`lottieRuntime`:** `MaskDefinitionJson`, `MaskHostLayerData`; **`CompLayerData`:** optional **`w` / `h` / `hasMask`**; exported **`MaskHostElement`**; **`shapePropertyFactoryTypes.ts`** (`ShapePropertyFactoryApi`) for **`getShapeProp`** calls; **`CVMaskLayerHost`** exported; **`MaskShapeProp`** exported from **`mask.ts`** for shared typing. **Track B:** **`CVMaskElement.getMaskProperty`** is a **class method** returning **`this.viewData[pos]`** (canvas stores a **`MaskShapeProp`** directly; SVG keeps **`viewData[i].prop`**). This replaces the old **`MaskElement.prototype`** alias, which was incorrect for masked-path text on canvas (**`TextAnimatorProperty`** expects **`mask.v`**, not **`undefined`** from a missing **`.prop`**).
...
- **`RenderableDOMElement` (typed):** **`@ts-nocheck` removed**; now a direct class that extends **`RenderableElement`** and defines lifecycle methods on the class body (`this: any` where needed).
- **Entry + standalone bundle:** **`main.ts`** — globals (**`setWebWorker`**, **`setLocationHref`**) use typed parameters (**`unknown`** / **`string`**). **`module.ts`** — file-level **`no-explicit-any`**; **`lottie`** as **`Record<string, any>`**; **`animationManager`** cast from default import; template **`standalone === true`** checks use **`(standalone as any) === true`**; **`queryString`** initialized; **`DataManager.completeAnimation`** call sites unchanged vs DOM **`AnimationItem`** (third arg **`undefined`** where required).
- **Worker canvas / animation overrides:** **`CanvasRendererWorkerOverride.ts`** — **`(CanvasRenderer.prototype as any).configAnimation`** with **`this: any`**. **`AnimationItemWorkerOverride.ts`** — **`(AnimationItem.prototype as any)`** for **`setParams`**, **`setData`**, **`includeLayers`**, segment loaders, **`configAnimation`**, **`checkLoaded`**, **`destroy`**, null stubs; **`completeAnimation(..., undefined)`** third argument matches **`DataManager`**. **`AnimationManagerWorkerOverride.ts`** — registry **`Record<string, any>`**, typed inner functions **`any`** where needed.
- **Main-thread animation + worker bridge:** **`animation/AnimationItem.ts`** — file-level **`no-explicit-any`**; instance fields; **`DataManager.completeAnimation`**, **`getRenderer`**, BM event helpers via **`.call`** (not **`new`**); **`(this as any)._cbs`** in **`destroy`**. **`animation/AnimationManager.ts`** — registry **`Record<string, any>`**, **`searchAnimations`** uses **`Element[]`**. **`worker_wrapper.ts`** — **`ProxyElement` / `CanvasElement`** typed fields; **`Style` / `CanvasContext`** with **`this: any`**; worker thread loads library via **`(self as any).lottie`**. **`removeEventListener`** in the worker passes **`entry.callback`**. Outer facade IIFE assigned to **`_lottieWorkerFacade`** (intentionally unused in-repo; consumed by **`tasks/build_worker.js`** string wrap).
- **Bundle modules (`src/modules/`):** **`main.ts`** aligned with **`module.ts`**. **`svg_light` / `canvas_light` / `html_light`** — side-effect registration only. **`full` / `svg` / `html` / `canvas`** — effect registration through **`registerSvgEffect` / `registerCvEffect`** casts. **`full_worker.ts`** — **`export {}`** placeholder.
